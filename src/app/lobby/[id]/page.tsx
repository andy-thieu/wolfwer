...
export default function Page() {
    ...
    const interval = setInterval(() => {
      setPlayers((prevPlayers) => {
        if (prevPlayers.length < 10) {
          ...
          return [...prevPlayers, { id: newPlayerId, name: `Spieler ${newPlayerId}`, isCreator: false }]
        }
        return prevPlayers
      })
    }, 5000)
  ...
  const toggleRole = (index: number) => {
    setRoles((prevRoles) => prevRoles.map((role, i) => (i === index ? { ...role, enabled: !role.enabled } : role)))
  }

  const updateRoleCount = (index: number, newCount: number) => {
    setRoles((prevRoles) => prevRoles.map((role, i) => (i === index ? { ...role, count: newCount } : role)))
  }
  ...
  const isCreator = players.find((player) => player.id === currentPlayerId)?.isCreator

  return (
    <div className="container mx-auto p-4">
      <h1 className="text-3xl font-bold mb-6 text-center">Wolfwer Lobby</h1>
        ...
        <Card className="w-full lg:w-1/3">
          ...
          <CardContent>
            <ul className="space-y-2">
              {players.map((player) => (
                <li key={player.id} className="bg-secondary p-2 rounded flex justify-between items-center">
                  <span>
            ...
            {!isCreator && (
              <Button className="w-full mt-4" variant="destructive" onClick={leaveRoom}>
                Lobby verlassen
              </Button>
            )}
          </CardContent>
          ...
          <Card>
            ...
            <CardContent className="space-y-6">
                ...
                {roles.map((role, index) => (
                  <div key={role.name} className="flex items-center justify-between">
                    <div className="flex items-center space-x-2">
                      <Switch checked={role.enabled} onCheckedChange={() => toggleRole(index)} disabled={!isCreator} />
                    ...
                    {role.enabled && (
                      <div className="flex items-center space-x-2">
                        <Button
                          variant="outline"
                          size="icon"
                          onClick={() => updateRoleCount(index, Math.max(1, role.count - 1))}
                          disabled={!isCreator}
                        >
                          -
                        </Button>
              ...
              <div className="space-y-2">
                ...
                <Select value={seerSeeRole} onValueChange={setSeerSeeRole} disabled={!isCreator}>
                  <SelectTrigger>
                    <SelectValue placeholder="Wähle eine Option" />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value="alignment">Nur Gesinnung (Gut/Böse)</SelectItem>
                    <SelectItem value="role">Genaue Rolle</SelectItem>
                  </SelectContent>
                </Select>
              </div>
              <div className="flex items-center space-x-2">
                <Switch checked={revealRoleOnDeath} onCheckedChange={setRevealRoleOnDeath} disabled={!isCreator} />
